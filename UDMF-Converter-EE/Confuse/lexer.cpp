...
static char *lexer_buffer_file(DWFILE *dwfile, size_t *len)
{
   ...
   {
      fprintf(stderr, "lexer_buffer_file: failed on file read (%d of %d bytes)\n",
              (int)foo, (int)size);
      exit(EXIT_FAILURE);
   }
...
static int lexer_state_hexescape(lexerstate_t *ls)
{
      ...
      (ls->c >= 'a' && ls->c <= 'f'))
   {
      ...
      {
         qstr += (char)strtol(hexchar.c_str(), nullptr, 16);
...
char *cfg_lexer_mustopen(cfg_t *cfg, const char *filename, const Wad *wad, int lumpnum, size_t *len)
{
   ...
   if(!(ret = cfg_lexer_open(filename, wad, lumpnum, len)))
   {
