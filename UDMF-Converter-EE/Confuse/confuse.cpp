...
static cfg_value_t *cfg_addval(cfg_opt_t *opt)
{
   ...
   cfg_assert(opt->values);
   opt->values[opt->nvalues] = (cfg_value_t *)calloc(sizeof(cfg_value_t), 1);
...
static cfg_opt_t *cfg_dupopts(cfg_opt_t *opts)
{
   ...
   ++n;
   dupopts = (cfg_opt_t *)calloc(sizeof(cfg_opt_t), n);
...
cfg_value_t *cfg_setopt(cfg_t *cfg, cfg_opt_t *opt, const char *value)
{
   ...
   case CFGT_MVPROP: // haleyjd
      ...
      val->section = (cfg_t *)calloc(sizeof(cfg_t), 1);
...
static int call_function(cfg_t *cfg, cfg_opt_t *opt, cfg_opt_t *funcopt)
{
   ...
   argv = (const char **)calloc(funcopt->nvalues, sizeof(char *));
...
static int cfg_pstate_expectargument(cfg_t *cfg, int level, cfg_pstate_t &pstate)
{
   ...
   else if(pstate.tok == CFGT_STR)
   {
      ...
      pstate.val->string = strdup(mytext);
...
cfg_t *cfg_init(cfg_opt_t *opts, cfg_flag_t flags)
{
   ...
   cfg = (cfg_t *)calloc(sizeof(cfg_t), 1);
   cfg_assert(cfg);
...
//
int cfg_parselump(cfg_t *cfg, const Wad &wad, const char *lumpname, int lumpnum)
{
   const Lump *lump = wad.FindLump(lumpname);
   if(!lump || lump->Data().empty())
      return CFG_SUCCESS;

   DWFILE dwfile; // haleyjd

   dwfile.openLump(wad, lumpnum);
...
void cfg_free(cfg_t *cfg)
{
   ...
   if(is_set(CFGF_ALLOCATED, cfg->flags))
   {
      free(cfg->namealloc);
      free(cfg->opts);
      free(const_cast<char *>(cfg->title));
   }
   else
      free(cfg->filename);
...
int cfg_include(cfg_t *cfg, cfg_opt_t *opt, int argc, const char **argv)
{
   ...
   if(!(data = cfg_lexer_mustopen(cfg, argv[0], nullptr, -1, NULL)))
      return 1;
