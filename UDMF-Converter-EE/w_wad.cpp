...
bool WadDirectory::addMemoryWad(openwad_t &openData, wfileadd_t &addInfo, int startlump)
{
   ...
   if(info_offset + header.numlumps * sizeof(filelump_t)  > openData.size)
   {
      ...
      {
         fprintf(stderr, "Failed reading directory for in-memory file\n");
         return false;
      }
   }
...
bool WadDirectory::addWadFile(openwad_t &openData, wfileadd_t &addInfo, int startlump)
{
   ...
   if(fread(&header, sizeof(header), 1, openData.handle) < 1)
   {
      ...
      else
      {
         ...
         fprintf(stderr, "Failed reading header for wad file %s\n", openData.filename);
   ...
   if(fread(fileinfo, length, 1, openData.handle) < 1)
   {
      ...
      {
         fclose(openData.handle);
         fprintf(stderr, "Failed reading directory for wad file %s\n", openData.filename);
         return false;
      }
   }
                                         ...
                                         wfileadd_t &addInfo, int startlump)
{
   ...
   if(!paths.isEmpty())
   {
      ...
      lumpinfo_t *lump_p = reAllocLumpInfo((int)paths.getLength(), startlump);
